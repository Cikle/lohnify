  ...
  Future<Map<String, CantonData>> fetchCantonRates() async {
    try {
      ...
      if (DateTime.now().difference(lastUpdate) < _updateInterval) {
        ...
        if (cached != null) {
          try {
            return Map<String, CantonData>.from(json.decode(cached).map(
                (key, value) => MapEntry(key, CantonData.fromJson(value))));
          } catch (e) {
      ...
      try {
        ...
        if (response.statusCode == 200) {
          ...
          return Map<String, CantonData>.from(data
              .map((key, value) => MapEntry(key, CantonData.fromJson(value))));
        } else {
          debugPrint(
              'Failed to fetch canton rates: HTTP ${response.statusCode}');
        }
      } catch (e) {
